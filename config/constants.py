...
# 核心任务
将用户提供的英文 JSON 数据，根据规则翻译成流畅、准确、符合游戏玩家用语习惯的简体中文，并严格按照指定格式输出。
...
# 原始输入数据
以下是需要处理的 JSON 格式的 Q&A 数据。你需要处理每个 JSON 对象中 `embeds` 数组内 `title` 和 `description` 字段的内容。
...
3.  分析 `description` 字段的文本。该文本通常由玩家的问题和开发者的回答两部分组成，分界线是 `------` 或 `***Replied to by...***`。剔除`[See reply here]`及后面的无关内容。确保形容词都已翻译没有遗留。确保漫威相关的专有名词都已翻译正确无误。
...
# 输出格式
你的最终输出必须是一个单独的 JSON 对象，其中包含一个名为 "translated_text" 的键。该键的值是一个字符串，包含了所有翻译和格式化后的 Q&A 内容。每个 Q&A 块之间用横线(———)隔开。
